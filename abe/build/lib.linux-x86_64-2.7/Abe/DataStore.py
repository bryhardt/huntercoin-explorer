...
#    def _is_descended_from(store, block_id, ancestor_id):
        ...
        try:
            store.sql(
                ...
                (block_id, store.hashin(b['hash']), store.intin(b['version']),
                 ...
                 len(b['transactions']), b['search_block_id'],b['algo']))
    ...
    def export_block(store, chain=None, block_hash=None, block_number=None):
        ...
        * proof_of_stake_generated
        ...
        (block_id, block_hash, block_version, hashMerkleRoot,
         ...
         satoshis, seconds, ss, total_ss, destroyed, num_tx,algo_solved) = (
            ...
            None if row[9] is None else store.binout_int(row[9]),
            None if row[10] is None else store.binout_int(row[10]),
            None if row[11] is None else store.binout_int(row[11]),
        ...
        for row in store.selectall("""
            SELECT tx_id, tx_hash, tx_size, txout_value, txout_scriptPubKey,name
              FROM txout_detail
             WHERE block_id = ?
             ORDER BY tx_pos, txout_pos
        """, (block_id,)):
            tx_id, tx_hash, tx_size, txout_value, scriptPubKey,name = (
                row[0], row[1], row[2], int(row[3]), store.binout(row[4]),row[5])
            tx = txs.get(tx_id)
            if tx is None:
                tx_ids.append(tx_id)
                txs[tx_id] = {
            ...
            if name is None:
                name = ""
            txout = { 'value': txout_value , 'name' : name }
    ...
    def import_tx(store, tx, is_coinbase, chain):
        ...
        for pos in xrange(len(tx['txOut'])):
            ...
            nameTx = chain.getNameTx(txout['scriptPubKey'])
            if (nameTx['name'] != ""):
                store.sql("""
                    INSERT INTO huc_name (
                        txout_id,name,type,color,tx_id
                    ) VALUES (?,?,?,?,?)""",
                        (txout_id,nameTx['name'],nameTx['type'],nameTx['color'],tx_id)
                    )
                if (nameTx['msg'] != ""):
                    store.sql("""
                        INSERT INTO huc_chat (
                            txout_id,name,msg,tx_id
                        ) VALUES (?,?,?,?)""",
                            (txout_id,nameTx['name'],nameTx['msg'].encode('utf-8'),tx_id)
                        )
                #print(nameTx)
                #sys.exit()
