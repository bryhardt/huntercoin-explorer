...
class Abe:
    ...
    def __call__(abe, env, start_response):
        ...
        if int((page['params'].get('api') or ['0'])[0]):
            page['template'] = JSON_TEMPLATE
            page['body'] = json.dumps(page['json'])
    ...
    def format_addresses_nolink(abe, data, dotdot, chain):
        if data['binaddr'] is None:
            return 'Unknown'
        if 'subbinaddr' in data:
            # Multisig or known P2SH.
            ret = [hash_to_address_nolink(chain.script_addr_vers, data['binaddr'], dotdot, text='Escrow'),
                   ' ', data['required_signatures'], ' of']
            for binaddr in data['subbinaddr']:
                ret += [' ', hash_to_address_nolink(data['address_version'], binaddr, dotdot, 10)]
            return ret
        return hash_to_address_nolink(data['address_version'], data['binaddr'], dotdot)    
    ...
    def handle_chain(abe, page):
        ...
        rows = abe.store.selectall("""
            SELECT b.block_hash, b.block_height, b.block_nTime, b.block_num_tx,
                   ...
                   b.block_total_ss,b.algo_solved
              FROM block b
        ...
        for row in rows:
            (hash, height, nTime, num_tx, nBits, value_out,
             seconds, ss, satoshis, destroyed, total_ss,algo_solved) = row
            nTime = int(nTime)
            ...
            jsBlock = {}
            jsBlock['bits'] = str(nBits)
            jsBlock['height'] = str(height)
            jsBlock['hash'] = abe.store.hashout_hex(hash)
            jsBlock['time'] = format_time(int(nTime))
            jsBlock['numOfTx'] = str(num_tx)
            jsBlock['valueOut'] = format_satoshis(value_out, chain)
            jsBlock['algoSolved'] = algo_solved
            jsBlock['diffFormated'] = util.calculate_difficulty(int(nBits))
            
            js['blocks'].append(jsBlock)
    ...
    def _show_block(abe, page, dotdotblock, chain, **kwargs):
        ...
        try:
            ...
            js['error'] = 'Not in correct format.'
            return

        if b is None:
            body += ['<p class="error">Block not found.</p>']
            js['error'] = 'Block not found.'
            return
        ...
        js['height'] = str(b['height'])
        js['version'] = str(b['version'])
        js['hashMerkleRoot'] = b['hashMerkleRoot']
        js['time'] = str(b['nTime'])
        js['timeFormatted'] = format_time(b['nTime'])
        js['bits'] = b['nBits']
        js['diff'] = util.calculate_difficulty(b['nBits'])
        js['diffFormatted'] = b['calcDiff']
        js['nonce'] = str(b['nNonce'])
        js['numOfTx'] = len(b['transactions'])
        js['valueOut'] = format_satoshis(b['value_out'], chain)
        js['fees'] = format_satoshis(b['fees'], chain)
        js['txs'] = []
        ...
        for tx in b['transactions']:
            ...
            jsTx['hash'] = tx['hash']
            jsTx['fees'] = format_satoshis(tx['fees'], chain)
            jsTx['size'] = tx['size']
            jsTx['ins'] = []
            jsTx['outs'] = []
            ...
            if tx is b['transactions'][0]:
                ...
                jsTx['generated'] = format_satoshis(b['generated'], chain)
                jsTx['totalFees'] = format_satoshis(b['fees'], chain)
            else:
                for txin in tx['in']:
                    jsIn = {}
                    ...
                    jsIn['address'] = abe.format_addresses_nolink(txin, page['dotdot'], chain)
                    jsIn['value'] = format_satoshis(txin['value'], chain)
                    jsTx['ins'].append(jsIn)
            ...
            for txout in tx['out']:
                ...
                jsOut['address'] = abe.format_addresses_nolink(txout, page['dotdot'], chain)
                jsOut['value'] = format_satoshis(txout['value'], chain)
                if txout['name'] != "":
                    jsTx['name'] = txout['name']
                jsTx['outs'].append(jsOut)
    ...
    def handle_block(abe, page):
        ...
        if not is_hash_prefix(block_hash):
            ...
            page['json']['error'] = 'Not a valid block hash.';
            return
    ...
    def handle_tx(abe, page):
        ...
        if not is_hash_prefix(tx_hash):
            ...
            page['json']['error'] = '<p class="error">Not a valid transaction hash.</p>'
            return
    ...
    def show_tx(abe, page, tx):
        ...
        def row_to_html(row, this_ch, other_ch, no_link_text):
            ...
            tx['value'] = format_satoshis(row['value'], chain)
            tx['address'] = abe.format_addresses_nolink(row, '../', chain)
            
            if row['binscript'] is not None:
                body += ['<td>', escape(decode_script(row['binscript'])), '</td>\n']
                tx['script'] = escape(decode_script(row['binscript']))
                tx['name'] = chain.getNameTx(row['binscript'])
                print(tx['name'])
            else:
                tx['script'] = ''
            body += ['</tr>\n']
        ...
        for tx_cc in tx['chain_candidates']:
            ...
            block = {}
            block['height'] = tx_cc['block_height']
            block['hash'] = blk_hash
            block['longest'] = tx_cc['in_longest']
            block['time'] = format_time(tx_cc['block_nTime'])
        ...
        js['numOfIns'] = len(tx['in'])
        js['totalIn'] =  format_satoshis(tx['value_in'], chain)
        js['numOfOuts'] = len(tx['out'])
        js['totalOut'] = format_satoshis(tx['value_out'], chain)
        js['size'] = tx['size']
        js['fee']= format_satoshis(0 if is_coinbase else
                                   (tx['value_in'] and tx['value_out'] and
                                    tx['value_in'] - tx['value_out']), chain)
    ...
    def handle_address(abe, page):
        ...
        if (not chains):
            if not int((page['params'].get('api') or ['0'])[0]):
                page['status'] = '404 Not Found'
            body += ['<p>Address not seen on the network.</p>']
            js['error'] = 'Address not seen on the network';
            return
        ...
        js['balance'] = format_amounts(balance,False)[0]
        ...
        js['numOfTxsIn'] = counts[0]
        js['numOfTxsOut'] = counts[1]
        js['received'] = format_amounts(received,False)[0]
        js['sent'] = format_amounts(sent,False)[0]
        js['txs'] = []
        body += ['</p>\n'
                 '<h3>Transactions</h3>\n'
        ...
        for elt in txpoints:
            ...
            tx['hash'] = elt['tx_hash']
            tx['blockHash'] = elt['blk_hash']
            tx['block'] = elt['height']
            tx['time'] = format_time(elt['nTime'])
            tx['value'] = value
            tx['balance'] = format_satoshis(balance[chain.id],chain)
            js['txs'].append(tx)
    ...
    def show_search_results(abe, page, found):
        ...
        if len(found) == 1 and not int((page['params'].get('api') or ['0'])[0]):
...
def hash_to_address_nolink(version, hash, dotdot, truncate_to=None, text=None):
